...
Sprint.init = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var p = new Sprint(data[i].project_id, data[i].description, data[i].name, data[i].id, data[i].date);
        Sprints[data[i].id] = p;
        newValues[i] = p;
    }
...
Comment.init = function(data) {
    var newValues = new Array();
    for (var i = 0; i < data.length; i++) {
        var p = new Comment(data[i].comment, data[i].task_id, data[i].id, data[i].time, data[i].user_id);
